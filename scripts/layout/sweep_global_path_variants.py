...
from ohbm2026.layout.poster_sequencing import sweep_global_path_diffusion_from_files
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Sweep global-path ordering variants, evaluate metrics on the overall sequence, and then split to blocks"
    )
    parser.add_argument(
        "--proposal",
    ...
    parser.add_argument(
        "--output-root",
        default="data/poster_layout/sequencing_benchmarks/semantic_layout_voyage31_global_path",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    sweep_global_path_diffusion_from_files(
