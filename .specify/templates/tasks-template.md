...
**Tests**: Include test or verification tasks for every behavior, contract,
pipeline, or UI change. Omit them only for pure documentation or content-only
work, and state that reason explicitly in the generated task list.
...
- [ ] T002 Create or refresh the repository-local virtual environment with `uv`
- [ ] T003 [P] Configure linting, formatting, and `.venv`-scoped commands
...
- [ ] T010 [P] Document secret boundaries and redaction expectations for external integrations
...
- Tests or explicit verification tasks MUST be written first and FAIL, or be
  shown to be missing, before implementation
