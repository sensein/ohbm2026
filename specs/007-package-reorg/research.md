...
**Net result for the `enrich/` package** (planned): 4 new submodules (`text.py`, `cache_paths.py`, `markdown_render.py`, `openai_compat.py`) carrying ~16 still-used symbols; ~40 symbols deleted.

**Net result (as implemented in US1)**: only 3 new submodules landed (`text.py`, `cache_paths.py`, `markdown_render.py`). The implementation-time consumer-fan-out audit showed the legacy multimodal helpers (`openai_chat_multimodal`, `openai_chat_multimodal_batch`, `resolve_openai_api_key`, `parse_jsonish_content`, `image_to_data_url`) had **zero real importers** across `src/`, `tests/`, and `scripts/` — Stage 2.1's production figure path uses `enrich/flex_tier.py` + `enrich/figures.py` via the OpenAI Responses API directly. Per FR-002, those symbols were deleted with `enrichment.py` rather than relocated into a now-empty `openai_compat.py`. The table rows pointing at `enrich/openai_compat.py` should be read as "(deleted)" — see US1 commit `3ea6f91` for the canonical record.
