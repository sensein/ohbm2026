...
- [X] T009 [P] [US3] Augment `tests/test_assets.py`: add `TestBatchedFetchHooks` (per-batch callback fires once per batch with correct submission_id list; per-record callback fires once per record with the (id, state) transition); `TestPerRecordStateTransitions` (pending → corpus_fetched → figures_in_progress → done; failed-retryable for figure errors; never transitions backwards); `TestPosterIdPropagation` (normalized abstract record carries the poster_id field populated from the upstream content response).
...
- [X] T011 [P] [US3] Augment `tests/test_cli.py`: assert `ohbmcli ingest` subcommand is REMOVED (importing it or invoking it raises a clear "renamed to fetch-abstracts" error); assert `ohbmcli fetch-abstracts` parses every option enumerated in `contracts/cli.md`; assert all CLI exit codes from `contracts/cli.md` are reachable via tests for the corresponding error conditions.
- [X] T012 [US3] Run `PYTHONPATH=src .venv/bin/python -m unittest tests.test_schema_diff tests.test_graphql_api tests.test_assets tests.test_fetch_stage tests.test_cli tests.test_artifacts -v` and confirm ALL new tests fail (red phase). Verify failure messages name the missing contract element, not just `ImportError`.
...
- [X] T021 [US2] Update `README.md` "End-To-End Workflow" section: remove the `ohbmcli ingest` invocation in Step 1; replace with the `scripts/run_fetch_abstracts.py` and `ohbmcli fetch-abstracts` invocations from `contracts/cli.md` + `quickstart.md`; add a sentence pointing at `docs/per-stage-pattern.md`; update the "Module Layout" section to list `fetch_stage.py`, `schema_diff.py`, and `exceptions.py`; update the "Main Outputs By Stage" entry for raw ingest to also list the schema artifact, provenance record, and (gitignored) checkpoint.
- [X] T022 [P] [US2] Update `CLAUDE.md`: refresh the "Module Layout" mentions; refresh the "Default pipeline state" section if any default changed; update the "Reading order" entry that points at the plan; ensure `CLAUDE.md`'s `## Non-negotiables` list is unchanged (no new principle introduced).
- [X] T023 [P] [US2] Update `docs/reproducibility-vision.md` Stage 1 paragraph (in the "Reproduction Ladder" section, Level 3): replace the `ohbmcli ingest` reference with the new entry; mention the persisted GraphQL schema and the per-stage pattern doc as new artifacts.
...
- [X] T024 Run the full test suite: `PYTHONPATH=src .venv/bin/python -m unittest discover -s tests -v`. Confirm green except the pre-existing unrelated `test_plot_poster_layout_floorplan` failure. No new failures, no skipped tests, no xfail markers (Principle VI, SC-006).
- [X] T025 Run `.specify/scripts/bash/constitution-check.sh --full` and confirm exit 0 (SC-007).
- [X] T026 Run `git status --porcelain` and `git diff --cached --name-only` after staging — verify NO file under `data/`, `export/`, `tmp/`, `archive/`, `memory/archive/`, or `.claude/` was tracked or staged (Principle II, CA-005, FR-008).
- [X] T027 Provenance fixture audit: grep test fixtures for absolute path patterns (`^/`) and home-prefix patterns (`~/` and `os.path.expanduser`) — assert none appear in any committed test or fixture under `tests/`. Use `git grep -nE "(\"|')(/Users/|/home/|~/)" -- tests/` and confirm any hit is a fixture asserting that such paths are REJECTED, not used as valid input (CA-008).
- [X] T028 Performance smoke (non-blocking): record wall-clock time of a fresh `scripts/run_fetch_abstracts.py` run against the live endpoint, compare to the most recent successful `ohbmcli ingest` time in `memory/summary.md` or recent runs. Allow up to 25% regression (loose envelope, not a primary SC). Record the result in the run's provenance record.
...
- [ ] T030 SC-001 walkthrough (manual): hand only the updated README's Stage 1 section to an unfamiliar contributor and have them run Stage 1 end-to-end (produce corpus + schema artifact + provenance record). Record any other documentation lookups they had to make (CLAUDE.md, plan, spec, contracts, quickstart, etc.). Acceptance per SC-001: zero docs lookups beyond the README's Stage 1 section.
...
- **Foundational (Phase 2)** — depends on Setup completion. BLOCKS all user-story phases. T003, T004, T006 can run in parallel. T005 is a sanity check (the FR-020 wiring it verifies was authored during planning and is already on disk in the spec/data-model commits), so it can run any time in Phase 2 without blocking other tasks. The augmented `test_artifacts.py` (T006) can be written in parallel with T003 because they touch different files.
