...
- [X] T021 [US1] Extend `src/ohbm2026/atlas_package/cli.py` argparser with `--semantic-index` / `--no-semantic-index` (default `--semantic-index`); `--semantic-cache-root` (default `data/cache/atlas-vectors`); `--semantic-model-id` (default `Xenova/all-MiniLM-L6-v2`); thread the values through to `AtlasBuildConfig`
...
- [X] T028 [US1] Modify `site/src/lib/components/NeuroscapeBrowsePanel.svelte` to mount the shared parameterised `SearchBar.svelte` (replaces the existing slim cluster-and-year-scoped input); wire the `✨ Semantic` toggle to `neuroscape_ranker.searchNeuroscape`; results flow into the existing result list via the same set-difference / ✨-badge logic as `/ohbm2026/` — **PARTIAL**: SearchBar mounted on /neuroscape/ + atlas-root via +page.svelte (the slim input lived there, not in NeuroscapeBrowsePanel); operator-aware filter (parseQuery + AND/OR/negation/phrase/id:N) wired into BOTH NeuroscapeBrowsePanel AND AtlasRootBrowsePanel — delivers FR-025 / FR-026 at the lexical level today. The ✨ Semantic toggle wiring to the ranker (semantic candidates fed into the result list, ✨-badge rendering) is the remaining follow-up; requires the ranker init from the loader's centroid+knn+fetcher deps in a Svelte component context.
