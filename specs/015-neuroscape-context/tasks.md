...
- [X] T030 [US4] Wire the `build-atlas-package` subcommand into `src/ohbm2026/cli.py` with the argparse surface documented in `contracts/cli-build-atlas-package.md`. Exit-code mapping per the contract (0 success, 2–7 per typed exception). (New module `src/ohbm2026/atlas_package/cli.py` with `build_parser()` + `main()` + `load_ohbm_corpus()` that joins `compose_recipe(['title','introduction','methods','results','conclusion'], model_key='neuroscape')` per-abstract Stage-2 vectors with `(submission_id, poster_id, title)` from `ohbm2026.parquet`'s abstracts row group. The `read_ohbm2026_state_key()` helper reads the renamed parquet's manifest. 11 tests cover the argparse surface, default values, every exit-code mapping, the provenance-write happy path, and the `--no-link-check` CI guard.)
- [X] T031 [P] [US4] Add `scripts/run_build_atlas_package.py` as a thin shim mirroring `scripts/run_enrich_abstracts.py`. README `--help` block must include every flag. (Created + chmod +x. Forwards `sys.argv[1:]` to `ohbm2026.atlas_package.cli.main`. The `--help` is generated by argparse from the documented surface — every flag listed.)
...
- [X] T034 [P] [US1] Write a failing vitest unit at `site/src/tests/unit/atlas_overlay.test.ts`: assert the `atlas_overlay` store defaults to `true`, persists writes to `localStorage` key `atlas_root.show_ohbm_overlay`, hydrates from a `"0"`/`"1"` localStorage value on init, and defaults to `true` on malformed input. (6 tests covering all four constraints plus a `toggleAtlasOverlay()` helper. Module-cache reset via `vi.resetModules()` so each test re-hydrates from fresh localStorage.)
- [X] T035 [P] [US1] Write a failing Playwright spec at `site/src/tests/e2e/atlas_root.spec.ts`: navigate to the atlas-root preview URL; assert (a) HTTP 200 + no `<meta http-equiv="refresh">` in the HTML, (b) the binary toggle is present and defaults to "on", (c) toggling off hides OHBM 2026 points but keeps the backdrop + legend, (d) the two header links target the right href, (e) clicking a NeuroScape point opens the DetailPanel with the right CTA href, (f) clicking an OHBM 2026 point opens it with the right CTA href, (g) lassoing a region produces a grouped result list with two collapsible sections whose counts sum to the lassoed total. (Spec gated by `ATLAS_ROOT_E2E=1` so it doesn't disturb the existing OHBM-2026-only e2e job. Three describe blocks: (a/b/d + title check) — assertions runnable now against any staged atlas-root build; (c/e/f/g) — `test.skip(true)` pending T045 UmapPanel adapter + T046 DetailPanel branch + T047 lasso; drift assertion — `test.skip(true)` pending T043. Deploy workflow extension T049 will flip the gate on for PR previews.)
...
- [X] T038 [P] [US1] Implement `site/src/lib/stores/atlas_overlay.ts`: a `localStorage`-backed Svelte writable store boolean. Default `true`, key `atlas_root.show_ohbm_overlay`. Tests from T034 MUST pass. (Mirrors the existing `showMap` pattern in `$lib/stores/selection`. Also exposes a `toggleAtlasOverlay()` helper for the component's `on:change` handler.)
- [X] T039 [P] [US1] Implement `site/src/lib/components/LandingPageHeader.svelte`: brand text "abstractatlas", two outbound subsite links ("Browse OHBM 2026 abstracts →" → `/ohbm2026/`, "Browse the NeuroScape PubMed atlas →" → `/neuroscape/`) per `contracts/atlas-root-ui.md`. (Both links carry `rel="external"` so SvelteKit's prerenderer treats them as cross-deployment — without it the atlas-root prerender fails because no `/ohbm2026/` route exists inside this bundle.)
- [X] T040 [P] [US1] Implement `site/src/lib/components/AtlasOverlayToggle.svelte`: binary checkbox bound to the `atlas_overlay` store, label "Show OHBM 2026 overlay".
- [X] T041 [P] [US1] Implement `site/src/lib/components/BackdropDensitySlider.svelte` per `contracts/atlas-root-ui.md`: slider 0.05–1.0, default 0.25, NOT persisted. (Parent owns `value` via `bind:value` so the same slider can drive multiple consumers (e.g. UmapPanel WebGL layer opacity).)
- [X] T042 [P] [US1] Extend `site/src/lib/data_package/loader.ts` with a new branch for `SITE_MODE === 'atlas-root'`: fetch `VITE_DATA_PACKAGE_URL_ATLAS`, decode the new outer rows (`clusters`, `neuroscape_backdrop_full`, `neuroscape_backdrop_decimated`, `ohbm_overlay`, `cross_pointers`), expose them via the existing `Map<path, ...>` envelope. (Dispatch keyed on the manifest's `schema_version`: `'atlas.v1'` → emits `data/atlas/{clusters,backdrop_full,backdrop_decimated,ohbm_overlay,cross_pointers}.json`; `'neuroscape.v1'` → emits `data/neuroscape/{articles,clusters,neighbors}.json` + `data/neuroscape/titles_index.{bin,meta.json}`; default `'abstracts.v2'` → existing OHBM 2026 envelope unchanged. Early return for atlas/neuroscape modes skips the OHBM-2026-only enrichment + standby_slots post-loop hydration. 3 vitest cases cover all three dispatch branches via a module-level `vi.mock('hyparquet')` factory.)
...
- [X] T044 [US1] Extend `site/src/routes/+page.svelte` with a `SITE_MODE === 'atlas-root'` branch per R-014: render LandingPageHeader + UmapPanel + AtlasOverlayToggle + ClusterLegend + BackdropDensitySlider; DO NOT render SearchBar / ResultList / FacetSidebar / CartDrawer. Gate via `{#if SITE_MODE === '…'}` blocks so build-time constant tree-shaking eliminates dead branches. The `ohbm2026` branch is unchanged. (Minimal branch in this slice: renders the new chrome (LandingPageHeader + AtlasOverlayToggle + BackdropDensitySlider) plus a placeholder for the scatter — the UmapPanel adapter lands in T045. Also extended `+layout.svelte` with a parallel `SITE_MODE === 'atlas-root'` branch that swaps the OHBM-2026-only chrome (`<title>`, header, tour CTA) for a minimal shell. Dead-branch tree-shaking confirmed by inspecting the ohbm2026 build's index.html — atlas-root markup absent.)
- [X] T045 [US1] Extend `site/src/lib/components/UmapPanel.svelte` with a `SITE_MODE === 'atlas-root'` data adapter: read from `atlas.parquet`'s `neuroscape_backdrop_full` or `_decimated` (per R-011 mobile detection) + `ohbm_overlay`; render NeuroScape points (cluster colour, outlined, smaller glyph) and OHBM 2026 points (foreground, larger, distinct outline) as separate WebGL layers. (Implemented as a new `AtlasUmapPanel.svelte` rather than overloading the existing `UmapPanel.svelte` — keeps the OHBM 2026 markup unchanged (FR-022) and the contract of each component focused. Reuses the existing `plotly.js-gl3d-dist-min` import (no new browser dep). Two `scatter3d` traces (cluster-coloured backdrop + outlined foreground) bound to `showOverlay` + `backdropOpacity` props; the +page.svelte atlas-root branch wires them to the `atlasOverlay` store + the BackdropDensitySlider. Mobile-detection (R-011 decimated backdrop default) deferred to a follow-up; first cut loads `neuroscape_backdrop_full` unconditionally. Click + lasso handlers deferred to T046/T047. Note: top-level `import AtlasUmapPanel` in `+page.svelte` means the component bytes ARE shipped in the ohbm2026 bundle even though the markup branch is dead — true bundle exclusion needs a dynamic import in T037's byte-identity work.)
...
- [X] T048 [P] [US1] Update `site/svelte.config.js` to read `SITE_MODE` from `process.env.SITE_MODE` (default `'ohbm2026'`) and propagate the per-mode base path: `''` for `atlas-root`, `/ohbm2026` for `ohbm2026`, `/neuroscape` for `neuroscape`. Document the matrix in a code comment. (Reads `VITE_SITE_MODE` (matching the env var Vite injects into the bundle so operators set ONE variable per mode); `SITE_MODE` accepted as a fallback for the env var rename. `BASE_PATH` continues to win when set explicitly so PR previews can override to `/pr-<N>/<subpath>`.)
