...
- Q: Are topics a separate analysis kind, or a per-cluster attribute of every clustering? → A: **Per-cluster attribute.** Generated clustering methods (communities and topic-model clusters) produce a row-aligned "topic list per cluster" alongside their cluster assignments; NeuroScape clusters reuse the published cluster metadata as labels. Topic modeling stays as one clustering METHOD but is no longer a standalone analysis kind.
...
- Q: How should community detection, centroid computation, and topic / cluster labeling be done? → A: **Match the published NeuroScape recipe where applicable.** kNN graph via FAISS `IndexFlatIP` (cosine on normalized embeddings) + symmetrize; Leiden with `CPMVertexPartition` and a resolution-parameter sweep; **spherical** (mean-on-hypersphere) centroids — convert to polar, take `mean_angle` per cluster, back to cartesian. Generated community/topic-cluster labels use the LLM-backed topic pipeline; NeuroScape cluster labels come from the published cluster table.
- Q: Where does the NeuroScape centroid table come from for `neuroscape_clusters` assignment? → A: **Pre-compute once from the published NeuroScape corpus.** NeuroScape ships `neuroscience_articles_1999-2023.csv` (461k articles × Cluster ID) + `HDF5/DomainEmbeddings/*.h5` shards (the Stage-2 projected 64-dim vectors). Stage 4 adds a one-off setup step (`scripts/derive_neuroscape_centroids.py`) that groups the published vectors by `Cluster ID` and applies the spherical-mean to write a centroid file at `data/inputs/neuroscape/centroids__<table_version>.npy` plus `cluster_table.csv`. Stage 4's `neuroscape_clusters` analysis kind reads this precomputed file and assigns each abstract to the nearest centroid in spherical (angular) distance.
...
- Q: Should NeuroScape cluster assignment treat the published clusters as a flat or hierarchical vocabulary? → A: **Flat 175-cluster vocabulary.** Stage 4 assigns OHBM abstracts to the nearest centroid of each original NeuroScape cluster and attaches the published cluster metadata; it does not infer parent-child hierarchy from similarity or citation graph metadata.
- Q: Should `ohbm2026.analyze` keep package-level public re-exports after deleting the flat `analyze.py` module? → A: **No package-level compatibility re-exports.** Every caller must import from explicit submodules such as `ohbm2026.analyze.umap` or `ohbm2026.analyze.storage`.
- Q: Should `neuroscape_clusters` run for both canonical inputs or only the manuscript/abstract input? → A: **Both canonical inputs for the NeuroScape domain embedding only.** The default matrix runs NeuroScape centroid assignment for `neuroscape_abstract` and `neuroscape_claims`; it does not run this assignment on original `voyage` embeddings.
- Q: How should the NeuroScape centroid bundle prove compatibility with Stage 3 `neuroscape` embeddings? → A: **Metadata comparison.** Centroid derivation writes source CSV hashes, HDF5 shard manifest hash, discovered cluster ids/count, and domain model checkpoint SHA; the runner compares the checkpoint SHA to Stage 3 `neuroscape` bundle provenance before assignment.

### Session 2026-05-16

- Q: Should the default input set include `methods` alongside `abstract` and `claims`? → A: **Yes — three canonical inputs.** Default `inputs = ("abstract", "claims", "methods")`. The `methods` input is the single per-component `methods__<state-key>` bundle for each model (a direct read, no recipe composition). Default matrix grows from 32 → **48 bundles** (5 models × 3 inputs × 4 kinds = 60 cells, 12 auto-skipped for non-`neuroscape` models on `neuroscape_clusters`). Operators can still filter via `--inputs abstract` or `--inputs claims methods`.
...
**Independent Test**: Run the canonical command against the current Stage 3 bundles for one input. Verify the bundle directory contains the four required analysis kinds (`projections`, `communities`, `neuroscape_clusters`, `topic_clusters`) and that each carries `metadata.json` + `provenance.json` plus the analysis-specific payload (coordinates, assignments, generated topic labels, or published NeuroScape cluster metadata). Verify an existing consumer (the UI export step) reads at least one of them without modification.
...
A researcher wants to label each Stage 3 abstract, for either canonical text view (manuscript/abstract or claims), with its corresponding NeuroScape published-domain cluster (the same flat `Cluster ID` vocabulary used in the published NeuroScape corpus). The pipeline reads the canonical NeuroScape cluster-centroid table derived from the published domain-embedding shards, computes the nearest centroid in the published embedding space for each abstract/input row, and persists the resulting cluster id + cluster label + angular/cosine distance per row.
...
1. **Given** a Stage 3 `neuroscape` domain-embedding bundle and the derived flat published-cluster centroid table, **When** the operator runs cluster assignment over the `neuroscape` manuscript/abstract and claims bundles, **Then** each row receives one cluster id from the original NeuroScape vocabulary, one published cluster label/metadata row, and one angular/cosine distance; the distribution of distances is recorded in the bundle's `metadata.json`.

2. **Given** the published centroid table is missing, lacks required metadata, or its domain model checkpoint SHA does not match the Stage 3 `neuroscape` bundle provenance, **When** the operator runs the assignment, **Then** the runner fails loudly with a typed error naming the missing or mismatched artifact.
...
**Independent Test**: After the reorganization, explicit submodule imports such as `from ohbm2026.analyze.umap import compute_umap_projection`, `from ohbm2026.analyze.storage import load_embedding_bundle`, and `from ohbm2026.embed.neuroscape import apply_stage2_model` work. Tests and scripts no longer import migrated helpers from package-level `ohbm2026.analyze`.
...
- The NeuroScape centroid metadata is missing required source hashes, cluster ids/count, or domain model checkpoint SHA, or its checkpoint SHA disagrees with the Stage 3 `neuroscape` bundle provenance. The runner MUST refuse to proceed and explicitly request the operator re-derive / re-pin the centroid file.
...
- An operator requests `neuroscape_clusters` for a source model that is not the `neuroscape` domain embedding (i.e., `model ∈ {voyage, minilm, openai, pubmedbert}`). The runner MUST skip the `(model, neuroscape_clusters)` pair by default and emit a structured `skipped` event recording the reason; with `--strict-matrix`, it MUST raise a typed `AnalysisError` naming the incompatible model + expected domain-embedding model.
...
- **FR-002**: The three canonical input sources for the default matrix MUST be: `<model_key>_abstract` (the manuscript recipe `title+introduction+methods+results+conclusion` composed via `embed.compose.compose_recipe` from the per-component bundles), `<model_key>_claims` (the single-component `claims` bundle for that model), and `<model_key>_methods` (the single-component `methods` bundle for that model). **The default model set MUST be all five embedding models — `voyage`, `minilm`, `openai`, `pubmedbert`, `neuroscape`.** Operators may filter via CLI (`--models …` / `--inputs …`); the default is "all". **Model-compatibility constraint for `neuroscape_clusters`**: the `neuroscape_clusters` analysis kind requires the already-projected NeuroScape domain embedding (`model == "neuroscape"`), because published NeuroScape cluster centroids live in that domain-embedding space, not the original Voyage embedding space. The matrix MUST automatically skip `(model, neuroscape_clusters)` pairs for `model ∈ {voyage, minilm, openai, pubmedbert}` and emit a structured `skipped` event for each canonical input, recording the reason. `(neuroscape, neuroscape_clusters)` consumes the Stage 3 domain-embedding bundle directly for all three canonical inputs (abstract/claims/methods). The runner MUST raise a typed error if an operator force-requests `neuroscape_clusters` on an incompatible model via `--strict-matrix`.
- **FR-003**: The system MUST produce four analysis-kind artifacts per `(model, input_source)` pair for v1: `projections` (carries both 2D and 3D UMAP coordinates), `communities` (kNN-graph community detection), `neuroscape_clusters` (nearest-centroid assignment in the published NeuroScape space), and `topic_clusters` (a topic-model-driven clustering, e.g., BERTopic-style). Generated clustering kinds (`communities`, `topic_clusters`) MUST emit a per-cluster `topics` artifact — a row-aligned list of representative keywords / phrases per cluster, derived via c-TF-IDF or equivalent over the cluster's member texts. The `neuroscape_clusters` kind MUST instead attach published NeuroScape cluster metadata from `cluster_table.csv` and MUST NOT infer a hierarchy from graph metadata.
...
- **FR-008**: The system MUST assign NeuroScape cluster ids by (a) consuming each `neuroscape` Stage 3 domain-embedding bundle directly, (b) loading a **precomputed centroid file** at `data/inputs/neuroscape/centroids__<table_version>.npy` (companion `cluster_table.csv` carries the flat NeuroScape `Cluster ID` vocabulary and published `Title` / `Description` / `Keywords` / `Focus` / `Dimensions` / related-cluster metadata from `neuroscience_clusters_1999-2023.csv`), (c) validating the centroid metadata against the Stage 3 `neuroscape` bundle provenance, and (d) computing the nearest centroid by **spherical** angular distance on the unit hypersphere. The centroid file is produced once via `scripts/derive_neuroscape_centroids.py`, which reads the published NeuroScape `HDF5/DomainEmbeddings/*.h5` shards + `CSV/neuroscience_articles_1999-2023.csv`, groups by `Cluster ID`, applies the spherical-mean recipe, and writes one centroid per discovered published cluster (175 rows for the current NeuroScape v1.0.1 snapshot). The centroid bundle metadata MUST record the source CSV hashes, HDF5 shard manifest hash, discovered cluster count, discovered cluster ids, centroid-table version, and domain model checkpoint SHA. Before assignment, the runner MUST compare the centroid metadata's domain model checkpoint SHA to the Stage 3 `neuroscape` bundle provenance and raise a typed error on mismatch. The output bundle records the centroid-table version, discovered cluster count, cluster ids, and the per-abstract angular-distance distribution in `metadata.json`.
- **FR-009**: Generated clustering analysis kinds (`communities`, `topic_clusters`) MUST emit a per-cluster `topics` artifact via a **two-stage hybrid pipeline**:
  ...
  When the LLM stage is skipped, the topics artifact contains the top-N c-TF-IDF phrases directly as `Keywords` and leaves `Title`/`Description`/`Focus` empty. The topics artifact is row-aligned with `cluster_id` so consumers can join cluster → topic-list in one read. `neuroscape_clusters` is excluded from this generated-topic pipeline because its labels come from the original NeuroScape cluster metadata.
...
- **FR-016**: The `analyze.py` module MUST be reorganized into a `analyze/` package containing: `analyze/stage.py` (orchestrator), `analyze/umap.py` (UMAP fitting + transform + bundle I/O), `analyze/clusters.py` (clustering + cluster benchmarks), `analyze/communities.py` (Leiden + CPM community detection over the FAISS kNN graph), `analyze/centroids.py` (spherical centroids + nearest-centroid assignment), `analyze/topics.py` (LLM-driven cluster-definition / topics artifact), `analyze/projections.py` (UMAP/t-SNE visualization HTML), `analyze/rollup.py` (single-file UI aggregate writer), and `analyze/storage.py` (bundle + cache atomic I/O). Stage-2 NeuroScape model code (currently in `analyze.py`) MUST physically move to `embed/neuroscape.py`, replacing the current re-export façade. **No backward-compat shim or package-level compatibility re-exports** — `ohbm2026.analyze` as a flat module is deleted, `analyze/__init__.py` remains minimal, and every caller (cli.py, scripts/, tests/) is updated to explicit submodule paths in this same change.
...
- **Input source**: A pair `(model_key, recipe_or_component)` that resolves to a single 2-D matrix of embeddings via `embed.compose.compose_recipe`. The canonical pair set for v1 is `{(model, "abstract"), (model, "claims"), (model, "methods")}` for each requested model.
- **Analysis kind**: One of `projections | communities | neuroscape_clusters | topic_clusters`. The `projections` kind carries BOTH 2D and 3D UMAP coordinates in one bundle (it is not split into separate `umap2d` / `umap3d` kinds). Generated clustering kinds (`communities`, `topic_clusters`) carry an associated topic-keyword list as a per-cluster attribute; `neuroscape_clusters` carries the published flat NeuroScape cluster metadata instead. Topics are NOT a standalone analysis kind. Each kind shares the metadata + provenance pattern.
...
- **CA-007**: NeuroScape centroid table version, source hashes, HDF5 shard manifest hash, discovered cluster ids, discovered cluster count, and domain model checkpoint SHA MUST be discovered at runtime from the artifact's own metadata, NOT matched against a hardcoded constant. Mismatches between the centroid metadata's checkpoint SHA and the Stage 3 `neuroscape` bundle provenance MUST raise.
...
- **SC-001**: A fresh Stage 4 run against the current Stage 3 bundles (corpus_state_key `f0c51e80dc0e`) produces all default-matrix annotation bundles in under **30 minutes** wall-clock on a single workstation. The default matrix is **48 bundles**: 15 `projections` (5 models × 3 inputs) + 15 `communities` (5 × 3) + 15 `topic_clusters` (5 × 3) + 3 `neuroscape_clusters` (`neuroscape` domain embedding × 3 inputs; `voyage`, `minilm`, `openai`, and `pubmedbert` are auto-skipped per FR-002's model-compatibility constraint).
...
- **SC-006**: NeuroScape cluster assignment over the 3244-abstract `neuroscape` domain embedding produces a non-degenerate distance distribution (mean cosine distance ≥ 0.15, std ≥ 0.05) — verifies the centroid lookup isn't a no-op.
...
- The NeuroScape centroid table is **derived once** from the published `HDF5/DomainEmbeddings/*.h5` + `CSV/neuroscience_articles_1999-2023.csv` + `CSV/neuroscience_clusters_1999-2023.csv` by a one-off setup script (`scripts/derive_neuroscape_centroids.py`); the resulting `centroids__<table_version>.npy` + `cluster_table.csv` + `metadata.json` are operator-supplied to Stage 4 via `--neuroscape-centroids data/inputs/neuroscape/`. The current NeuroScape v1.0.1 artifact exposes 175 flat clusters (`Cluster ID` 0–174); implementation still discovers the count and ids from the files at runtime. The derivation needs `h5py` (added to the `[analysis]` extra in `pyproject.toml`).
- "Topic modeling" is a **two-stage hybrid** for generated clusters: spaCy phrase extraction + class-based TF-IDF scoring (local, deterministic, no API) feeds a 60-phrase candidate list per cluster, then an optional one-call-per-cluster LLM pass groups/re-ranks the phrases and emits `{Title, Description, Focus}`. Passing `--skip-llm-topics` produces a fully-local run; the canonical pipeline runs the LLM pass for generated community/topic clusters because the UI consumes the richer labels. NeuroScape cluster labels use the published cluster metadata and do not invoke the LLM. The LLM operates on phrase lists not raw abstracts, so the per-call token budget is ~3k (vs ~30k for a raw-abstract approach) — full-corpus cost at `gpt-5.4-mini` flex is ~$0.15 for ~100 clusters.
...
- The annotation matrix produces **48 per-bundle artifacts by default**: 15 `projections` (5 models × 3 inputs) + 15 `communities` (5 × 3) + 15 `topic_clusters` (5 × 3) + 3 `neuroscape_clusters` (only `neuroscape` domain embeddings are compatible with the published NeuroScape centroids; voyage / minilm / openai / pubmedbert are auto-skipped). Operators can request additional `(model, input)` combinations or skip kinds via CLI flags; `--strict-matrix` turns the auto-skip into a typed error.
