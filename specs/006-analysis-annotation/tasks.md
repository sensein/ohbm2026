...
- [X] T097 Run the live quickstart against the current Stage 3 corpus `f0c51e80dc0e`: `ohbmcli analyze-matrix` → 48 bundles + 1 rollup pair landed under `data/outputs/analysis/` (15 projections + 15 communities + 15 topic_clusters + 3 neuroscape_clusters; 12 `bundle_skipped` events for voyage/minilm/openai/pubmedbert × {abstract, claims, methods} on the neuroscape_clusters kind). Validated 2026-05-16 with the joblib-parallel orchestrator (`--n-jobs -1`, default).
...
- [X] T098 Validate **SC-001** (< 30 min wall-clock) — live default-matrix wall-clock = **536 s (8.93 min)** with `--n-jobs -1` on 10 cores (2.0× speedup vs serial 1084 s baseline). PASS by 3.4× margin.
- [X] T099 Validate **SC-002** (< 60 s cached re-run) — immediate rerun of `ohbmcli analyze-matrix` against the same Stage 3 corpus completed in **0.66 s** (all 48 bundles cache-hit, 0 written). PASS by 91× margin.
- [X] T100 Validate **SC-005** — all 15 community bundles at seed=42 satisfy n≥12 (range 15–56) and largest_community_share≤14% (well under 30%). Includes voyage_abstract (n=17, largest=12.5%) and voyage_claims (n=19, largest=13.3%). PASS.
- [X] T101 Validate **SC-006** — NeuroScape angular-distance distribution per bundle: `neuroscape_abstract` mean=1.021/std=0.178, `neuroscape_claims` mean=1.012/std=0.188, `neuroscape_methods` mean=1.011/std=0.169. All three ≥0.15 mean and ≥0.05 std. PASS.
- [X] T102 Validate **SC-003** + **SC-007** — determinism re-run identical: full matrix re-run with the same config produces `run_state_key=7c5d01dce1ed` (same as the original full run) and hits cache for all 48 bundles in 0.66 s, confirming the canonical state-key + bundle outputs are byte-identical modulo timestamps. Test sweep: `unittest discover -s tests` runs 583 tests, 1 pre-existing failure (`test_plot_poster_layout_floorplan`, unrelated to Stage 4); all 97 `test_analyze_*` tests pass.
...
- [X] T109 Final live `ohbmcli analyze-matrix --skip-llm-topics` run — verified that the local-only path produces the expected topic shape (Title/Description empty, Keywords populated from top-N c-TF-IDF). Bundle `voyage_claims/communities__35f0ab799e3f/topics.json` after `--invalidate communities --skip-llm-topics`: 19/19 clusters have empty Title and Description; 19/19 have Keywords from the local spaCy + c-TF-IDF pipeline. Zero LLM calls fired (runner short-circuits `build_topics_llm_adapter` when `skip_llm_topics=True`).
